...
data=/home/chenyang/chenyang_space/ctc_model_new/data
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  # Test model, 5please specify the model you want to test by --checkpoint
...
if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
  ...
  decode_checkpoint=$dir/avg_30.pt
      ...
      # align
  align_dir=$dir/alignment
  mkdir -p $align_dir
  python wenet/bin/alignment.py --gpu 0 \
      --config $dir/train.yaml \
      --data_type $data_type \
      --input_file data/test/data.list \
      --checkpoint $decode_checkpoint \
      --dict $dict \
      --result_file $align_dir/text \
      --gen_praat 
...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
  ...
  ctc_weight=0.5
  reverse_weight=0.0
  test_data_list=/home/chenyang/chenyang_space/data/aishell3_test_fa/data.list #这里我们换成自己的测试集
  test_data_list=/home/chenyang/chenyang_space/data/aishell_test_conformer_attention

  decode_modes="attention"

  for mode in ${decode_modes}; do
  {
    test_dir=$test_data_list/test_${mode}
    mkdir -p $test_dir
    python wenet/bin/alignment_attention.py --gpu 0 \
      --mode $mode \
      --config $dir/train.yaml \
      --data_type $data_type \
      --test_data /home/chenyang/chenyang_space/data/aishell3_test_fa/data.list \
      --checkpoint $decode_checkpoint \
      --beam_size 1 \
      --batch_size 1 \
      --penalty 0.0 \
