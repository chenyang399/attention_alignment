...
# # # 使用torch.cat()函数在最前面增加一个值
#             value_to_add  =torch.tensor([value_to_add]).to(device)
#             target = torch.cat([value_to_add, target])
...
#             print(target)
            ...
            print(target)
            print("target")
            if key==['BAC009S0764W0122']:
                ctc_probs=ctc_probs.cpu()
                target=target.cpu()
                ctc_probs=ctc_probs.transpose(0,1)
                ...
                plt.plot(ctc_probs[0], linestyle='-', color='red', label='blank')
                plt.plot(ctc_probs[target[0]], linestyle='-', color='blue', label='标')
                plt.plot(ctc_probs[target[1]], linestyle='-', color='yellow', label='杆')
                plt.plot(ctc_probs[target[2]], linestyle='-', color='green', label='房')
                
                pic_path="/home/chenyang/chenyang_space/wenet/wenet/examples/aishell/s0/pic/3.png"
