...
class ASRModel(torch.nn.Module):
    ...
    def recognize(
        ...
        # unsqueeze为解压变为（b,1,maxlen,encoder_dim）  repeat里为每个维度重复的次数 变为（b,beamsize,maxlen,encoder_dim）
        # view 类似reshape
        encoder_mask = encoder_mask.unsqueeze(1).repeat(
            1, beam_size, 1, 1).view(running_size, 1,
                                     maxlen)  # (B*N, 1, max_len)
        # print('encoder_mask')
        # print(encoder_mask) 打印出来都是true
        hyps = torch.ones([running_size, 1], dtype=torch.long,
                          device=device).fill_(self.sos)  # (B*N, 1)
                        #  blank 0 unkonwn 1 sos/eos 4232 所以fill的是4232
        ...
        # zerolike复制原来的tensor但是内容全为0，endflag得到的是 (B*N, 1)的全0
        cache: Optional[List[torch.Tensor]] = None
        # print(maxlen)大多是80-130一类，表示的是贞的数目
        ...
        #   最终得到了logp是每个的字的概率分布，cache是经过的decode得到的东西每次forward one step就增加一维
            # 2.2 First beam prune: select topk best prob at current time
            top_k_logp, top_k_index = logp.topk(beam_size)  # (B*N, N)
            # print('lopk')
            # print(top_k_logp)
            # print('index')
            # print(top_k_index)   #将4?243个字的概率的前是个拿出来，在beamsize=10的时候每个可能性都拿出前十个
            # 这里的index指的是每个字对应的数字，因为原本是4023维
            ...
            # print('offset_k_index')
            # print(offset_k_index) 
            # 这里就是每次保留十个可能的结果，每次一个可能的结果后面最可能的十个结果，所以一共100中可能里面选出来十个
            # print('scores')
            # print(scores)
            ...
            # 就相当于一次按照chacheindex的一次排序，把cache里面六个输出每个的beamsize个结果按照index进行排序
    ...
    def forward_attention_decoder(
        ...
        # 在第一个维度上重复，实际上就是有几个候选的ctc结果重复几次
    ...
    def align_attention(
        ...
        for i in range(1, maxlen + 1):
            # 这里的maxlen就是个摆设，他是encoder的帧数，最对输出的字不超过帧数，具体结束的时候其实用的是eos
            # Stop if all batch and all beam produce eos
        ...
        #   最终得到了logp是每个的字的概率分布，cache是经过的decode得到的东西每次forward one step就增加一维
            ...
            # 变成每次加上真实的值，这样就是forcealignment了
            if i-1>=target_len:
                hyps=torch.cat((hyps,(torch.tensor([[self.eos]])).to(device)),dim=1)
            else:
                hyps=torch.cat((hyps,target[0][i-1].view(-1,1)),dim=1)
